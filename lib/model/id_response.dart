...
class IDResponse {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "IDResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "IDResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<IDResponse> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<IDResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<IDResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = IDResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
