...
class ApiClient {
  ...
  void setAccessToken(Token? accessToken) {
    ...
    if (_accessToken != null) {
      _defaultHeaderMap['Authorization'] =
          'Bearer ${_accessToken!.accessToken}';
    } else {
      _defaultHeaderMap.remove('Authorization');
    }
  }

  String? get accessToken => _accessToken?.accessToken;
  ...
  // Static method to set access token on the default API client
  static void setDefaultAccessToken(Token? accessToken) {
    defaultApiClient.setAccessToken(accessToken);
  }

  // Static method to get access token from the default API client
  static String? getDefaultAccessToken() {
    return defaultApiClient.accessToken;
  }

  // Static method to clear access token from the default API client
  static void clearDefaultAccessToken() {
    defaultApiClient.clearAccessToken();
  }
  ...
  Future<Response> invokeAPI(
    ...
    // Override with access token if available (this ensures latest token is used)
    if (_accessToken != null) {
      // print("Using Access Token: $_accessToken");
      // Add or update the Authorization header with the Bearer token
      headerParams['Authorization'] = 'Bearer ${_accessToken!.accessToken}';
    }
