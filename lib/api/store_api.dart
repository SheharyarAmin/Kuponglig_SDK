...
class StoreApi {
  ...
  Future<Response> getStoreStatsApiV1StoreStatsStoreIdDateGetWithHttpInfo(String storeId, DateTime date,) async {
    // ignore: prefer_const_declarations
    final path = r'/api/v1/store/stats/{store_id}/{date}'
      .replaceAll('{store_id}', storeId)
  ...
  ///
  /// * [DateTime] date (required):
  Future<StoreStatsModel?> getStoreStatsApiV1StoreStatsStoreIdDateGet(String storeId, DateTime date,) async {
    final response = await getStoreStatsApiV1StoreStatsStoreIdDateGetWithHttpInfo(storeId, date,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'StoreStatsModel',) as StoreStatsModel;
  ...
  ///
  /// * [StoreModelBase] storeModelBase (required):
  Future<Response> updateStoreApiV1StoreStoreIdPutWithHttpInfo(String storeId, StoreModelBase storeModelBase,) async {
    // ignore: prefer_const_declarations
    final path = r'/api/v1/store/{store_id}'
      .replaceAll('{store_id}', storeId);
  ...
  ///
  /// * [StoreModelBase] storeModelBase (required):
  Future<StoreModel?> updateStoreApiV1StoreStoreIdPut(String storeId, StoreModelBase storeModelBase,) async {
    final response = await updateStoreApiV1StoreStoreIdPutWithHttpInfo(storeId, storeModelBase,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
